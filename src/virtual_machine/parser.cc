...
static const char *const yytname[] =
{
  ...
  "JUMP_1", "STOP", "REG", "LABEL", "ERROR", "$accept", "input", "line", YY_NULLPTR
};
...
yy_symbol_value_print (FILE *yyo,
                       yysymbol_kind_t yykind, YYSTYPE const * const yyvaluep, vector< pair<int,int> > & program)
{
  FILE *yyoutput = yyo;
  YY_USE (yyoutput);
  YY_USE (program);
...
yy_reduce_print (yy_state_t *yyssp, YYSTYPE *yyvsp,
                 int yyrule, vector< pair<int,int> > & program)
{
...
yydestruct (const char *yymsg,
            yysymbol_kind_t yykind, YYSTYPE *yyvaluep, vector< pair<int,int> > & program)
{
  YY_USE (yyvaluep);
  YY_USE (program);
  if (!yymsg)
